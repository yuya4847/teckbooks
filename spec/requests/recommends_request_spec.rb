RSpec.describe "Recommends", type: :request do
  ...
  describe "リコメンド機能のrequestが動作する" do
    ...
    describe "#recommend_open_modal" do
      ...
      context "リコメンドしたユーザーがいない場合" do
        it 'リコメンド完了のリクエストが成功すること' do
          get recommend_user_display_path(), params: { review_id: good_review.id, recommend_user_datas: "" }, xhr: true
          expect(response).to have_http_status(200)
        end
      ...
      context "リコメンドしたユーザーがいる場合" do
        it 'リコメンド完了のリクエストが成功すること' do
          get recommend_user_display_path(), params: { review_id: good_review.id, recommend_user_datas: "#{second_user.id},#{third_user.id}" }, xhr: true
          expect(response).to have_http_status(200)
        end
        ...
        it 'リコメンド済は削除され、未リコメンドはリコメンドされること' do
          second_recommend.save
          expect do
            get recommend_user_display_path(), params: { review_id: good_review.id, recommend_user_datas: "#{third_user.id}" }, xhr: true
